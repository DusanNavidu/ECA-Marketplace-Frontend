...
export default function AdminLayout({ children }) {
  ...
  const menuItems = [
    ...
    { path: '/admin/sellers', label: 'Manage Sellers', icon: '👥' },
    { path: '/admin/buyers', label: 'Manage Buyers', icon: '🛒' },
    { path: '/admin/catalog', label: 'All Catalog Items', icon: '📦' },
  ];

  return (
    <div className="min-h-screen bg-bg-light flex font-sans overflow-hidden">
      ...
      >
        <div className="h-20 flex items-center justify-between px-6 border-b border-gray-100">
          ...
          <Link to="/">
            <div className="shrink-0 flex items-center justify-center gap-2 cursor-pointer transition-transform hover:scale-105">
              ...
              <span className="font-extrabold text-2xl bg-clip-text text-transparent bg-linear-to-r from-gray-800 to-gray-600 hidden sm:block md:block">
                Marketplace
              </span>
            </div>
          </Link>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link 
                ...
                to={item.path} 
                onClick={() => setIsMobileMenuOpen(false)}
                className={`flex items-center gap-3 p-3.5 rounded-xl transition-all duration-200 font-medium ${
                  isActive 
        ...
        <div className="p-5 border-t border-gray-100 bg-gray-50/50">
          <Button variant="outline" className="w-full shadow-sm bg-white" onClick={handleLogout}>
            Log Out
          </Button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden w-full relative">
        ...
        <header className="h-20 bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-100 flex items-center justify-between px-4 sm:px-8 shrink-0 z-30">
          ...
          <button 
            className="md:hidden p-2 -ml-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-primary"
          ...
          <div className="flex items-center gap-3 ml-auto">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-xs text-gray-500">admin@eca.com</p>
            </div>
            <div className="w-10 h-10 bg-linear-to-br from-primary to-blue-400 rounded-full flex items-center justify-center text-white font-bold shadow-md ring-2 ring-blue-100">
              A
            </div>
          </div>
        </header>
